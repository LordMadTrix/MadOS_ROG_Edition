#!/bin/bash
# ==============================================================================
# MadOS ROG Edition 4.0 - 32_stealth_privacy.sh
# ==============================================================================
...
echo -e "\n${RED}╔══════════════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${RED}║${NC} 🛡️ ${WHITE}${BOLD}Phase 32 : Activation du Mode Discrétion v4.0 (Stealth-Mode Privacy)${NC}"
echo -e "${RED}╚══════════════════════════════════════════════════════════════════════════╝${NC}\n"
...
# 4. Nettoyage Télémétrie (KDE Plasma + GNOME fallback)
echo -e "    ${WHITE}├─ [UI] Désactivation des services de localisation et de télémétrie...${NC}"
REAL_USER="${SUDO_USER:-$USER}"

# KDE Plasma — DrKonqi crash reporter + KUserFeedback
user_run kwriteconfig6 --file drkonqirc --group General --key AutomaticSendingEnabled false 2>/dev/null || true
user_run kwriteconfig6 --file KDE/UserFeedback.conf --group Global --key FeedbackLevel 0 2>/dev/null || true
# GNOME fallback (silencieux si non présent)
sudo -u "$REAL_USER" dbus-launch gsettings set org.gnome.system.location enabled false 2>/dev/null || true
sudo -u "$REAL_USER" dbus-launch gsettings set org.gnome.desktop.privacy report-technical-problems false 2>/dev/null || true
...
if [ "$1" == "on" ]; then
  ...
  sudo -u "${SUDO_USER:-$USER}" kwriteconfig6 --file "$HOME/.config/kwinrc" --group "NightColor" --key "Active" "true" 2>/dev/null
  ...
  sudo -u "${SUDO_USER:-$USER}" kwriteconfig6 --file "$HOME/.config/kwinrc" --group "NightColor" --key "Active" "false" 2>/dev/null
